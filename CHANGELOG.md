...
- **`recall`'s `spread: true` no longer ignores the `since` bound.** Spreading follows relationships one hop out and reads embeddings directly, so it never passed through either filtered query — and because spread results are merged, re-sorted by score and cut to `limit`, damped out-of-window hits could push the in-window rows out of the answer entirely. On the regression fixture: a bound of 2026-08-25 with `limit: 2` returned two observations from 2020 and none of the rows asked for. That is the same "nothing new happened" outcome as the bug above, in a shape that reads as a healthy result set rather than an empty one. Related: an empty-string `since` — and a truncated one like `2026` or `2026-08`, which used to work as a lexicographic prefix — now errors instead of being silently treated as a bound — it is almost always an unset variable, and the old behaviour handed the caller everything, which reads as "all of this is new".
